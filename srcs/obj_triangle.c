...
/*   Updated: 2023/07/04 15:39:31 by kakiba           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	point_in_triangle(t_vec3 pt, t_vec3 v1, t_vec3 v2, t_vec3 v3)
{
	bool	b1;
	bool	b2;
	bool	b3;
...
t_intersection	get_intersection_plane_tr(\
	const t_ray ray, t_triangle *triangle)
{
	t_obj			obj;
	t_plane			plane;
...
t_intersection	get_intersection_triangle(const t_ray ray, const t_obj *obj)
{
	...
	if (intersection.does_intersect == false)
		return (intersection);
	if (point_in_triangle(intersection.position, \
		triangle->p1, triangle->p2, triangle->p3) == false)
